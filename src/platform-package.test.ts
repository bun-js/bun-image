...
test("packed package installs and exposes the bun-image executable", async () => {
  ...
  const pack = Bun.spawnSync(
    ["bun", "pm", "pack", "--destination", root, "--quiet"],
    { cwd: `${import.meta.dir}/..` },
  )
  ...
  if (!archive)
    throw new Error(`bun pm pack did not report an archive: ${packOutput}`)
  ...
  const conversion = Bun.spawnSync(
    [`${installRoot}/node_modules/.bin/bun-image`, input, output],
    { cwd: installRoot },
  )
...
test("Linux reports clipboard input as unavailable", () => {
  ...
  const result = Bun.spawnSync(
    ["bun", "run", "src/index.ts", "--clipboard", "-"],
    {
      cwd: `${import.meta.dir}/..`,
    },
  )
